...
## Monorepo: Content Studio (`content-studio/`)

The repo may also include a **Content Studio** Next.js app under `content-studio/`.

### Local dev (Content Studio)

From the **repo root** (after `npm install` inside `content-studio/`):
...
Use **http://localhost:3000** by default. If the tab is blank or errors: free port 3000, or clear `content-studio/.next` and retry.

### Vercel (Content Studio)

Set **Root Directory** to **`content-studio`** when deploying that app. Root **`vercel.json`** may still delegate to `./content-studio` when present.
...
├── content-studio/                # Optional monorepo app (see section above)
├── .github/
│   └── workflows/
