 ...
 *
 * When `anthropicKeyForBrief` is set (Content Studio Claude key), **Claude** runs this step; the image
 * is still generated by **OpenAI GPT Image** or **Gemini** via `provider` + `apiKey` in LeftPanel.
 */
export async function buildVisualBrief(
  ...
  anthropicKeyForBrief?: string | null,
): Promise<string> {
...
---`;
  ...
  const claudeKey = anthropicKeyForBrief?.replace(/[^\x20-\x7E]/g, "").trim() ?? "";
