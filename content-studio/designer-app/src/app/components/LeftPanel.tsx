...
import {
  buildDesignerPostLayoutContext,
  buildCriticalBackgroundParagraph,
...
export function LeftPanel({ onContentGenerated, onSettingsChange, onGenerateVisual, hasContent, provider, apiKeyRaw, headerMode, mode, setMode, settings: settingsFromProps, registerPreviewToolbar, isEmbed = false }: LeftPanelProps) {
  ...
  /** Embed: hydrate local variations from lifted `settings` once; avoids resetting on every parent patch. */
  const embedHydratedRef = useRef(false);
  ...
  useEffect(() => {
    ...
    if (settingsFromProps.theme) {
      setTheme(settingsFromProps.theme);
    }
  ...
  /* ── Layout for structure-generation chat — same POST LAYOUT / bg rules as image prompt (white / trns / gradient) ── */
  const buildLayoutContextForStructure = () => {
    const postSizeIdForStructure =
      headerMode === "designer" && settingsFromProps?.postSizeId
        ? settingsFromProps.postSizeId
  ...
  const handleGenerateStructure = async () => {
    ...
    const layoutCtx = buildLayoutContextForStructure();
  ...
  const generateSingleVisual = async (content: GeneratedContent, themeId: string, variationIdx: number, sourceImage?: string | null, omitContentTextInImage?: boolean, visualBrief?: string | null): Promise<string | null> => {
    ...
    const aspectExtra =
      isDesignerTrnsPostSize(postSizeIdForPrompt) && whiteForPrompt
        ? " User selected 1:1 trns + White bg — use opaque white full canvas (not transparency)."
        : isDesignerTrnsPostSize(postSizeIdForPrompt)
    ...
    const designerTrnsClause =
      headerMode === "designer" && isDesignerTrnsPostSize(postSizeIdForPrompt) && !settingsFromProps?.designerWhiteBg
...
[
  ...
  useEffect(() => {
    ...
    const propVisual = settingsFromProps?.visualImage;
    const showGeneral =
      mode === "general" &&
      !loading &&
      (variations.length > 0 ||
  ...
  return (
    <div className="bg-muted border-r border-border p-4 sm:p-5 overflow-y-auto" style={{ maxHeight: "100vh" }}>
      ...
      {/* API status indicator (hidden in embed — parent app owns account chrome) */}
      {!isEmbed && !apiKey && (
      ...
      {mode === "general" && (
        <>
          ...
          <CollapsibleSection
            title="Templates"
            icon="🎨"
            defaultOpen={headerMode === "designer" || isEmbed}
            badge={selectedThemes.length > 1 ? `${selectedThemes.length}` : undefined}
          >
