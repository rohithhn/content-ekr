...
export function buildDesignerPostLayoutContext(
  ...
  if (designerWhiteBg) {
    ...
    bgLine =
      "Background: one unified light field using **brand colors from BRAND COLOR GUIDANCE** at low opacity, or a soft neutral tint — not a second company’s orange/pink unless the brief specifies it.";
  }
...
export async function buildDesignerOpenAiImagePrompt(opts) {
  const {
    ...
    variationIdx = 0,
    layoutOverrides = {},
    /** Serialized Content Studio brand (colors, gradients, logos as absolute URLs) — drives BRAND COLOR GUIDANCE */
    studioBrandPayload = null,
  } = opts;
  ...
  const brandColorRules =
    buildDesignerBrandColorRules(studioBrandPayload) ?? DEFAULT_DESIGNER_BRAND_COLOR_RULES;
  const paletteAccentGuidance = studioBrandPayload?.colors
    ? "Use the brand primary gradient and accent colors from BRAND COLOR GUIDANCE above for icons and highlights; red/green rules below still apply where the brief specifies danger/success."
